...
digital_folder = config.get('general', 'mp3_folder')
                ...
                if row[0] == self.serial:
                    ...
                    if self.split_point:
                        if self.name[-2] is not '_':
                            metadata_errors.append('Error! Split file expected for {0}'.format(self.name))
                            return
                    ...
                    if 'std' not in self.option:
                        ...
                        if not self.cues:
                            metadata_errors.append('Error! No markers found in audio for Deluxe item {0}'.format(self.name))
                            return
                        if not tracks:
                        ...
                        if len(tracks) is not len(self.cues) - 1:
                            metadata_errors.append('Error! Item {0} expected {1} tracks, found {2} in audio file.'.format(
            ...
            if item.compilation_cd_counter:
                ...
                for each_item in [each_item for each_item in item_list if each_item.compilation_cd_counter == item.compilation_cd_counter and \
                    ...
                    for track in each_item.tracks:
                        if track.title and not track.title.lower().startswith('track'):
                            compilation_track_title = '{0} - {1}'.format((each_item.artist or each_item.album), track.title)
                        elif each_item.artist and each_item.album:
                            compilation_track_title = '{0} - {1}'.format(each_item.artist, each_item.album)
                        elif each_item.artist or each_item.album:
                            compilation_track_title = str(each_item.artist or each_item.album)
                        else:
...
if __name__ == '__main__':
    ...
    if metadata_errors:
        ...
        raise Exception('Metadata errors found! See above.')
