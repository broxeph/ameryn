import sys, numpy, struct, warnings, wave
...
input_filename_list = ['00458_clean']
input_filename_list_range = range(458, 461)
input_path = 'c:/temp/splitty temp/'
output_path = 'c:/temp/splitty temp/split/'
...
def split_file(input_filename):
    f = input_path+input_filename+'.wav'
    print f
    f_markers = read_markers(f)
    print f_markers
    ...
    for cue_num in range(len(f_markers)):
        print 'Cue #'+ str(cue_num) +':', f_markers[cue_num]
        # If marker #1 at wave start
        if cue_num == 0 and f_markers[cue_num] < 1000:
            print 'Marker #1 skipped: @ < 1000 frames'
            continue
        else:
            region_num += 1
            region_name = output_path+input_filename+'_'+str(region_num)+'.wav'
            region = wave.open(region_name, 'w')
            region.setparams(ifile_params)
            print str(cue_num)+'/'+str(len(f_markers))
            # If marker #1 after wave start
            if cue_num == 0:
                print 'Marker #1 after file start'
                region.length = f_markers[cue_num]
            # If final marker and before wave end
            elif cue_num == len(f_markers) - 1 and f_markers[cue_num] < ifile.getnframes() - 1000:
                # Write final region
                region.length = f_markers[cue_num] - f_markers[cue_num - 1]
                print 'Region #'+ str(region_num) +':', region.length, '->', region_name
                region.writeframes(ifile.readframes(region.length))
                region.close()
                # Write final-cue-to-EoF region
                print 'EoF marker missing'
                region_num += 1
                region_name = output_path+input_filename+'_'+str(region_num)+'.wav'
                region = wave.open(region_name, 'w')
                region.setparams(ifile_params)
                region.length = ifile.getnframes() - f_markers[cue_num]
                print 'Region #'+ str(region_num) +':', region.length, '->', region_name
                region.writeframes(ifile.readframes(region.length))
                region.close()
                break
