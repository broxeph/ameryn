...
def main(input_resplit_list=False):
    ...
    #utils.export_csv('resplit-list')
    #utils.export_csv('resplit-track-lists')
    ...
    if input_resplit_list:
        #utils.export_csv(['re-split_list', 'resplit-track-lists', 'resplit-artists'])
        '''
        with open(config.resplit_track_lists_path, 'r') as resplit_track_lists:
            config.split_folder_list = os.listdir(config.split_folder)
            server_directory_list = os.listdir(config.server_split_folder)
            ...
            for row in rowdata:
                ...
                if not filename_matches:
                    filename_matches = [os.path.join(config.server_split_folder, each) for each in server_directory_list if each.startswith(query)]
                if not filename_matches:
                ...
                matches_count += 1
        '''
        with open(config.resplit_artists_path, 'r') as resplit_artists:
            config.split_folder_list = os.listdir(config.split_folder)
            server_directory_list = os.listdir(config.server_split_folder)
            rowdata = [row for row in csv.reader(resplit_artists)]
            for row in rowdata:
                query = row[0]              
                # Check for filename query matches
                filename_matches = [os.path.join(config.split_folder, each) for each in config.split_folder_list if each.startswith(query)]
                ...
                for match in filename_matches:
                    audio = FLAC(os.path.join(config.split_folder, match))
                    audio['artist'] = row[1].decode('unicode-escape')
                    audio['album'] = row[2].decode('unicode-escape')
                    audio['composer'] = row[3].decode('unicode-escape')
                    audio.save()
                matches_count += 1
    # Done? Great!
...
if __name__ == '__main__':
    main(input_resplit_list=config.input_resplit_list)