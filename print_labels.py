...
def export_digital(item):
    ...
    if 'std' in item.option:        
        if config.just_add_tags:
            ...
            digital_file.export(out_f=item.digital_file_path, format=item.digital_ext, bitrate=item.bitrate, parameters=item.export_parameters,
                tags={'title':(item.album or item.artist), 'artist':item.artist, 'albumartist':item.artist, 'album':(item.album or item.artist)},
                id3v2_version='3')
        # Add cover art
...
def make_digital_files(item_list, send_emails=True):
...
def make_cd_pdf(item_list):
    ...
    # Draw CD labels
    c = canvas.Canvas(os.path.join(config.pdf_folder, 'cd-labels.pdf'), pagesize=(150 * mm, 320 * mm))
...
def make_front_pdf(item_list):
    ...
    if not front_cover_list:
        ...
        return
    logging.info('front_cover_list: ({0}): {1}'.format(len(front_cover_list), [item.name for item in front_cover_list]))
...
def make_back_pdf(item_list):
    def add_back_cover(position):
        ...
        c.drawImage(config.watermark_path, 157 * mm, 25*mm + image_adjustment, width=16 * mm, height=11 * mm)
        ...
        for size, padding in [(11,2), (11,1), (10,2), (10,1), (9,1), (9,0), (8,1), (8,0), (8,-1), (7,0), (7,-1), (7,-2), (6,0), (6,-1), (6,-2)]:
            # Add table data
            table_data = []
    ...
    for order in set([item.order for item in item_list]):
        ...
        for item in order_compilation_items:
            if not compilation_image:
                ...
                item.image_path = os.path.join(config.images_folder, item.image) + '.jpg'
...
def make_cover_image(item):
    if item.image_counter and item.compilation_cds != 1:
        ...
        d.ellipse(circle_bounds, fill=(255,255,255,128))
        ...
        label_path = os.path.join(config.cd_tracks_folder, item.name) + '_' + counter + '.jpg'
...
def burn_cds(item_list, test=False):
    """
    ...
    for item in burn_cd_list:
        ...
        if test:
            # Fake JRQ file
            jrq_path = os.path.normpath(config.ptburn_folder + '/' + job_name + '.xxx')
        else:
            jrq_path = os.path.normpath(config.ptburn_folder + '/' + job_name + '.JRQ')
        with open(jrq_path, 'w') as f:
            lines = []
            ...
            if 'std' in item.option and not item.long:
                ...
                if item.compilation_counter:
                    ...
                    cd_items = [each_item for each_item in item_list if each_item.order == item.order and \
                        each_item.compilation_cd_counter == item.compilation_cd_counter and not each_item.copy_counter]
                else:
                    cd_items = [item]
                ...
                for cd_item in cd_items:
                    ...
                    for i, track in enumerate(cd_item.tracks, 1):
                        ...
                        digital_track_path = os.path.join(config.cd_tracks_folder, digital_track_name) + '.wav'
                        logging.debug('Track {0} | {1} -> {2}'.format(i, track.duration, digital_track_name))
                        # Split, export track
...
def main():
    ...
    if config.just_add_tags:
        ...
        input_list = [each_file for each_file in os.listdir(config.tracked_folder) if (each_file.lower().endswith('.wav') and 
            each_file[:5] in config.input_orders)]
    elif config.input_items:
        print 'Input items:', config.input_items
        input_list = [each_file for each_file in os.listdir(config.tracked_folder) if (each_file.lower().endswith('.wav') and \
            any(each_item_input in each_file for each_item_input in config.input_items))]

    # Class-ify item input list
    print 'Item-izing input list...'
    ...
    if config.burn_cds_toggle: burn_cds(item_list, test=False)
    order.print_order_notes(item_list, refresh_csv=False)
    if config.digital_files_toggle: make_digital_files(item_list, send_emails=True)
