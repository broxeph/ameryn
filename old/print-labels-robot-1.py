            ...
            for row in rows:
                if row[0] == self.serial:
                    ...
                    with closing(wave.open(self.path, 'r')) as audio:
                        ...
                        if self.duration > 4680:
                            self.long = True
                            if 'std' not in self.option and not self.split_point:
                    ...
                    if 'std' not in self.option or self.long:
                        ...
                        if len(self.cues) < 3 and 'std' not in self.option:
...
def make_cd_pdf(item_list):
    ...
    # Change to make CD .jpg for burn_cds()
    cd_label_list = []
...
def format_list(item_list):
    # Split long items; add compilation, image counters
    new_items = []
    for item in item_list:
        # Split >78m std/trk CDs
        if item.long:
            item2 = deepcopy(item)
            item.side = 1
            item.tracks = item.tracks[:item.split_point]
            item2.side = 2
...
def burn_cds(item_list):
    # Working: <78m std, tracked, doubles, compilations
    # In progress: image counters
    # NYI: 1/4s
    burn_cd_list_full = format_list([item for item in item_list if 'cd' in item.format])
    # For compilations, remove items after the first
    burn_cd_list = [item for item in burn_cd_list_full if item.compilation_cd_item_counter <= 1]
    #burn_cd_list = burn_cd_list_full
    for each in burn_cd_list:
        print each.name
        print ' - side:', each.side
        print ' - compilation_counter:', each.compilation_counter
        print ' - compilation_cd_item_counter:', each.compilation_cd_item_counter
        print ' - compilation_cd_counter:', each.compilation_cd_counter
        print ' - compilation_cds:', each.compilation_cds
        print ' - image_counter:', each.image_counter
    logging.info('CDs: ({0}): {1}'.format(len(burn_cd_list), [item.name for item in burn_cd_list]))
    for item in burn_cd_list:
        # Create CD label jpg
        #
        job_name = item.name + '-' + str(item.side) if item.side else item.name
        with open(os.path.normpath(ptburn_folder + '/' + job_name + '.xxx'), 'w') as f:
            lines = []
            lines.append('JobID = ' + job_name)
            lines.append('ClientID = ' + os.environ['COMPUTERNAME'])
            lines.append('DeleteFiles = ' + ('NO' if 'std' in item.option else 'YES'))
            if 'std' not in item.option or item.album or item.artist:
                # Write disc CD text
                if item.compilation_items > 1 and item.compilation_cds == 1:
                    ...
                    lines.append('CDTextDiscTitle = {0} (Disc {1})'.format(item.album, item.image_counter))
                else:
                    lines.append('CDTextDiscTitle = ' + item.album)
                if item.compilation_items > 1:
                    lines.append('CDTextDiscPerformer = Various')
                else:
                    lines.append('CDTextDiscPerformer = ' + item.artist)
            ...
            if 'std' in item.option and not item.long:
                ...
                if item.compilation_counter:
                    ...
                    cd_items = [each_item for each_item in burn_cd_list_full if each_item.order == item.order and \
            ...
            #lines.append('Copies = ' + item.copies)
            lines.append('RejectIfNotBlank = YES')
            #lines.append('TestRecord = YES')
            lines.append('PrintLabel = D:/Sextus share/02537-00001.jpg')
            #lines.append('PrintLabel = ' + item.image_path) #("The print file does not exist."?)
            for line in lines:
                f.write((line + '\n').encode('utf8'))
