...
orders_input = [order.strip() for order in config.get('print-labels', 'orders_input').strip().split(',')]
items_input = [item.strip() for item in config.get('print-labels', 'items_input').strip().split(',')]
...
def main():
    ...
    if just_add_tags:
        input_list = [each_file for each_file in os.listdir(audio_folder) if os.path.splitext(each_file.lower())[1][1:] in digital_formats]
    elif whole_folder:
        input_list = [each_file for each_file in os.listdir(audio_folder) if each_file.lower().endswith('.wav')]
    elif some_orders:
        input_list = [each_file for each_file in os.listdir(audio_folder) if (each_file.lower().endswith('.wav') and 
    ...
    item_list = [Item(each_file) for item in input_list]
