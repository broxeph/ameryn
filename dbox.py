...
def send_dbox_email(order_id, wc_client=None, dbox_client=None, smtp_server=None, test=False):
    ...
    for item in order['line_items']:
        if 'lp' in item['name'].lower() or '45' in item['name'] and 'lp' not in items_text and '45' not in items_text:
            items_text = 'record' if item['quantity'] == 1 else 'records'
        elif 'cassette' in item['name'].lower() and 'cassette' not in items_text:
            if items_text:
                items_text += ' and '
            items_text += 'cassette' if item['quantity'] == 1 else 'cassettes'
        for meta in item['meta']:
            # Update digital format (mp3/FLAC)
            if meta['key'] in ('mp3-download', 'format'):
                digital_format = meta['value']
        if 'cd' in item['name'].lower():
            cds += item['quantity']
    total_items = sum([item['quantity'] for item in order['line_items'] if 'cassette' in item['name'].lower() or 'lp' in item['name'].lower() or \
        '45' in item['name']])
    ...
    emailtext += 'This link will be usable for 30 days. If you have any questions, feel free to reply to this email or call us at (844) AMERYN-1.'
    ...
    if test:
        # Don't send email nor modify order status
        print ' - - - - - - - - - - - -'
        print message
        print ' - - - - - - - - - - - -'
        return None
...
def send_all(order_ids, test=False):    
