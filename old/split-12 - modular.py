...
def split_item(item, DIGITAL_FOLDER=mp3_folder):    
    ...
    for i, track in enumerate(item.tracks):
        digital_track_path = '{0}_{1}.{2}'.format(os.path.join(DIGITAL_FOLDER, item.name), str(i + 1), item.digital_ext)
        ...
        if 'wav' not in item.digital_ext:
            ...
            tags = {'title':track.title, 'artist':item.artist, 'album':item.album, 'track':(i + 1)}
...
def split_file(input_filename, export_format='flac', bitrate='192k', tracks=None, artist='Unknown', album='Untitled', cover=None):
    ...
    for marker_num, marker in enumerate(file_markers):
        ...
        if export_format is not 'wav':
            ...
            mp3_tags = {'title':mp3_title, 'artist':artist, 'album':album, 'track':(marker_num + 1)}
