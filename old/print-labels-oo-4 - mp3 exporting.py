                ...
                if row[0] == self.serial:
                    ...
                    if 'std' not in self.option:
                        ...
                        if len(tracks) is not len(self.cues) - 1:
                            raise Exception('Error! Item {0} expected {1} tracks, found {2} in audio file.'.format(self.name, len(tracks), len(self.cues) - 1))                        
...
def export_digital(item):
    if 'std' in item.option:
        ...
        digital_track.export(out_f=digital_track_path, format=item.digital_ext, bitrate='192k', tags={'title':item.album, 'artist':item.artist, 'album':item.album})
        # Add cover art
        if item.thumb and item.digital_ext is 'mp3':
...
def make_digital_files(item_list):    
    ...
    logging.warning('digital_file_list: ({0}): {1}'.format(len(digital_file_list), [item.name for item in digital_file_list]))
...
def make_cd_pdf(item_list):
    ...
    if not front_cover_list:
        ...
        return
    print 'cd_label_list: ({0}): {1}'.format(len(cd_label_list), [item.name for item in cd_label_list])
    ...
    def add_back_cover(position):
        ...
        for size, padding in [(11,2), (11,1), (10,2), (10,1), (9,1), (9,0), (8,0), (8,-1), (7,-1), (7,-2), (6, -1), (6,-2)]:
            ...
            for track in item.tracks:
                #table_data.append((Paragraph((str(track.num)), styles['track_title']), Paragraph(track.title, styles['track_title']), track.duration))
