...
def make_back_pdf(item_list):
    def add_back_cover(position):
        ...
        # Left spine label
        x = (118*mm - pdfmetrics.stringWidth(spine, 'Garamond', fontsize))/2 + 21.7*mm + image_adjustment
        y = -37.3 * mm
        c.drawString(x, y, spine)
        # Right spine label
        c.rotate(180)
        x = (118*mm - pdfmetrics.stringWidth(spine, 'Garamond', fontsize))/2 - 139.7*mm - image_adjustment
        y = 178.7 * mm
        c.drawString(x, y, spine)
        c.restoreState()
        # Draw track listing
        table = Table(item.tracks, colWidths=[6 * mm, 100 * mm, 10 * mm])
        fontsize = 11
        padding = 2
        if len(item.tracks) > 14:
            fontsize = 10
        if len(item.tracks) > 16:
            fontsize = 9
            padding = 1
