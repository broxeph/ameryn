		...
		if hasattr(result,'tracklist'):
			...
			if len(tracklist) == tracks_count:
				...
				if double_type in ['1/4', '1/3'] and filename_matches[0][-7] not in 'abcd':
					...
					if double_type == '1/3':
						sort_key = ['a', 'c', 'b', 'd']
					positions = [t['position'] for t in tracklist]

					key_counter = 0
					zippy = zip(filename_matches, positions)
					filename_matches
					for key in sort_key:
						for match, position in zippy:
							if position.lower().startswith(key):
								new_filename = match.rsplit('-', 1)[0]+'_'+position+match[-5:]								
								old_filename_fullpath = os.path.join(flac_directory, filename_matches[key_counter])
								filename_matches[key_counter] = new_filename
								new_filename_fullpath = os.path.join(flac_directory, new_filename)
								print key_counter, old_filename_fullpath, '->', new_filename_fullpath
								os.rename(old_filename_fullpath, new_filename_fullpath)
								key_counter += 1
				...
				if len(discogs_lengths) == tracks_count:
				...
				print 'Result '+str(i + 1)+': Nope! ('+str(len(tracklist))+' != '+str(tracks_count)+')'
	else:
