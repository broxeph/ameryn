...
def strip_file(input_filename):
	...
	if os.path.exists(os.path.splitext(input_filename_fullpath)[0] + '.pkf'):
		shutil.move(os.path.splitext(input_filename_fullpath)[0] + '.pkf', \
			os.path.join(archive_path, os.path.splitext(input_filename)[0] + '.pkf'))
...
for input_path in input_paths:
	...
	print 'Input filenames (folder: {0}): {1}'.format(input_path, input_filename_list)
...
# List silent files for re-recording
if silent_files:
	print
