...
def add_compilation_counters(item_list):
    ...
    for order in set([item.order for item in item_list]):
        ...
        for item in order_items:
            if '45' in item.media_type:
                compilation_counter += 1
                logging.info('Woohoo! {0} is single #{1} for order {2}!'.format(item.name, compilation_counter, item.order))
...
def make_digital_files(item_list):    
    ...
    if not JUST_ADD_TAGS:
        for i, item in enumerate([item for item in digital_file_list if digital_file_list.count(item.digital_file_name) > 1]):
            item.digital_file_name += '_{0}'.format(str(i))
            item.digital_file_path = os.path.join(DIGITAL_FOLDER, item.digital_file_name) + '.' + item.digital_ext
...
def make_back_pdf(item_list):
    def add_back_cover(position):
        ...
        logging.warning('Processing back cover for item {0}'.format(item.name))
        # Image positioning
        if position == 'top':
