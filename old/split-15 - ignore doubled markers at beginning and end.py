...
input_filename_series = config.getboolean('split', 'input_filename_series')
input_whole_folder = config.getboolean('split', 'input_whole_folder')
input_filename = config.get('split', 'input_filename')
input_filename_list_start = config.get('split', 'input_filename_list_start')
input_filename_list_end = config.get('split', 'input_filename_list_end')
input_filename_list_range = (input_filename_list_start, input_filename_list_end)
input_path = config.get('general', 'server_clean_path')
split_path = config.get('split', 'split_path')
export_format = config.get('split', 'export_format')
mp3_folder = config.get('general', 'mp3_folder')
log_level_input = config.get('split', 'log_level')
log_level = logging.WARNING if log_level_input == 'warning' else logging.INFO
pool_processing = config.getboolean('split', 'pool_processing')
...
def split_item(item, digital_folder=mp3_folder, dropbox_move=True, bitrate='192k'):
    ...
    for i, track in enumerate(item.tracks):
        ...
        if 'wav' not in item.digital_ext:
            ...
            digital_track.export(out_f=digital_track_path, format=item.digital_ext, bitrate=bitrate, tags=tags)
            # Add cover art
...
def split_file(input_filename, export_format='flac', bitrate='192k', tracks=None, artist='', album='', cover=None):
    ...
    if file_markers[-1] - file_markers[-2] < 200000:
        print 'Doubled end-marker deleted.'
        del file_markers[-2]
    if file_markers[1] - file_markers[0] < 100000:
    ...
    for marker_num, marker in enumerate(file_markers):
        ...
        region_name = os.path.join(split_path, region_basename)
        ...
        if export_format is not 'wav':
            ...
            mp3_tags = {'title': mp3_title, 'artist': artist, 'albumartist': artist, 'album': album,
                        'track': (marker_num + 1)}
            mp3_fname = os.path.join(split_path, (os.path.splitext(region_basename)[0] + '.' + export_format))
